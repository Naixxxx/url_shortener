...
class LinkServiceTest {
  ...
  static class TestConfig implements AppConfig {
    ...
    public Path userUuidFile() {
      return Path.of("build/tmp/user.uuid");
    }
  }
  ...
  void differentUsers_getDifferentCodes_forSameUrl() {
    ...
    LinkService svc =
        new LinkService(repo, new TestConfig(), new CodeGenerator(repo), notifier, clock);
  ...
  void limit_blocksAfterReached_andNotifiesOnce() {
    ...
    assertThrows(
        LinkLimitReachedException.class, () -> svc.resolveForRedirect(l.getCode())); // блок
    assertEquals(1, notifier.limit);
  }
  ...
  void ttl_expires_andDeletes_onResolve() {
    ...
    LinkService svc0 =
        new LinkService(repo, new TestConfig(), new CodeGenerator(repo), notifier, clock0);

    ShortLink l = svc0.create("u1", "https://example.com", 5);

    Clock clock1 = Clock.fixed(t0.plusSeconds(11), ZoneOffset.UTC);
    LinkService svc1 =
        new LinkService(repo, new TestConfig(), new CodeGenerator(repo), notifier, clock1);
